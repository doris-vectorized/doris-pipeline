...
#include "pre_aggregation_sink_operator.h"
...
Status PreAggSinkOperator::prepare(RuntimeState* state) {
    RETURN_IF_ERROR(Operator::prepare(state));
    _agg_node->_child_return_rows =
            std::bind<int64_t>(&PreAggSinkOperator::get_child_return_rows, this);
    return Status::OK();
}

Status PreAggSinkOperator::open(RuntimeState* state) {
    RETURN_IF_ERROR(Operator::open(state));
    RETURN_IF_ERROR(_agg_node->alloc_resource(state));
    return Status::OK();
}

bool PreAggSinkOperator::can_write() {
    // sink and source in diff threads
    return _agg_context->has_enough_space_to_push();
}

Status PreAggSinkOperator::sink(RuntimeState* state, vectorized::Block* in_block, bool eos) {
...
Status PreAggSinkOperator::close(RuntimeState* state) {
    // for poc
...
OperatorPtr PreAggSinkOperatorBuilder::build_operator() {
    return std::make_shared<PreAggSinkOperator>(this, _agg_node, _agg_context);
}

// use final aggregation source operator
bool PreAggSinkOperatorBuilder::is_sink() const {
    return true;
}

bool PreAggSinkOperatorBuilder::is_source() const {
    return false;
}
} // namespace doris::pipeline